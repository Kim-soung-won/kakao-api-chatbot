...
> ⚠️ **왜 파일인가 (설계 변경):** 처음엔 서버 저장소 없이 카카오 네이티브 `context` 왕복만으로
> 이력을 실어 나르려 했으나, **실서비스에서 context 왕복이 확인되지 않아**(RAG 검색이 항상
> "대화 이력이 없어요") botUserKey 기반 파일 저장소로 전환했다. 파일은 docker 볼륨에 적재돼
> 호스트에서 그대로 확인·수집되고 컨테이너 재생성에도 보존된다. → 결정 d10 (C4 designIntent).
