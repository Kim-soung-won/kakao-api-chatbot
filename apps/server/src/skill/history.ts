 ...
 *
 * 저장 방식 결정 배경 (실측 확정):
 *   당초 카카오 네이티브 `context` 왕복만으로 이력을 나르려 했으나, 실카톡 진단 결과
 *   응답에 심은 output context가 다음 요청으로 **왕복되지 않음**을 확인했다(요청의 최상위
 *   `contexts`가 매번 빈 배열). 대신 `userRequest.user.id`(botUserKey)가 64자 해시로 안정적이라,
 *   이를 키로 한 **파일 저장소**(history-store.ts)로 전환했다. 파일은 docker 볼륨
 *   (captured-requests/)에 적재돼 호스트에서 그대로 확인·수집된다.
 */
